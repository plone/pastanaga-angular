...
GLYPHS_DIR = os.path.join(os.path.dirname(__file__), '../projects/ngx-pastanaga/glyphs')
...
for filename in os.listdir(GLYPHS_DIR):
    if filename.endswith('.svg'):
        ...
        glyph.set('fill', root.get('fill'))
        glyph.set('viewBox', root.get('viewBox'))
        glyph.set('stroke-width', root.get('stroke-width'))
        glyph.set('stroke', root.get('stroke'))
