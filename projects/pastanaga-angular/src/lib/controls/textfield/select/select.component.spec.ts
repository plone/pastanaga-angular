...
describe('SelectComponent', () => {
  ...
  const thenSelectHasValue = (value: any, label: any) => {
    ...
    expect(spectator.query('.pa-select-value')?.innerHTML?.trim()).toEqual(label);
  };
  ...
  it('should have an id, a name and a label', () => {
    ...
    expect(spectator.query('.pa-field-label')?.innerHTML?.trim()).toEqual('Label');
  });
  ...
  it('should display placeholder or selected option label', fakeAsync(() => {
    ...
    expect(spectator.query('.pa-select-value')?.innerHTML?.trim()).toEqual('');
